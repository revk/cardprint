...
static void
dump (const void *buf, size_t len, const char *tag)
{
   if (!debug)
      return;
...
static const char *
get_settings (j_t j, unsigned char nvr)
{
...
static const char *
card_load (unsigned char newposn, unsigned char immediate, unsigned char flip, unsigned char filminit)
{
...
static const char *
card_move (unsigned char newposn, unsigned char immediate, unsigned char flip, unsigned char filminit)
{
...
static const char *
transfer_flip (unsigned char immediate)
{
...
static const char *
transfer_eject (unsigned char immediate)
{
...
static const char *
transfer_return (unsigned char immediate)
{
...
static const char *
send_panel (unsigned char panel, unsigned int len, void *data, unsigned char buffer)
{
   if (panel >= FILMS)
...
static const char *
print_panels (unsigned char panels, unsigned char immediate, unsigned char buffer)
{
...
{                               // list the readers
   ...
   while (*p && p < e)          // && !error)
   {
      if (!readeric && strstr (p, "HID Global OMNIKEY 3x21 Smart Card Reader"))
         ...
         readeric = strdup (p);
      else if (!readerrfid && strstr (p, "OMNIKEY") && strstr (p, "CardMan") && strstr (p, " 5121 "))
         readerrfid = strdup (p);
      else if (debug)
...
static void
card_txn (int txlen, const unsigned char *tx, LPDWORD rxlenp, unsigned char *rx)
{
   if (error)
      return;
...
static ssize_t
ss_write_func (void *arg, void *buf, size_t len)
{
   return SSL_write (arg, buf, len);
}

static ssize_t
ss_read_func (void *arg, void *buf, size_t len)
{
      ...
      if (print)
      {
         ...
         } else
         {
            ...
            const char *print_side (j_t panel)
            {
               ...
               {
                  const char *d;
                  if ((d = j_get (panel, "YMC")) || (d = j_get (panel, "CMY")) || (d = j_get (panel, "C"))
                      || (d = j_get (panel, "Colour")))
                     add (d, FILM_YMC);
